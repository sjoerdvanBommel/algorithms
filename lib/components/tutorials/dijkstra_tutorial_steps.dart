...
class DijkstraTutorialSteps {
  ...
  TutorialStep getTutorialStep(int codeStep) {
    if (codeStep == 0) {
      start = null;
      return TutorialStep(
      ...
      return TutorialStep(
        ...
        goBack: () => nodes.forEach((element) => element.distance = 1 << 32),
      );
    } else if (codeStep == 3) {
      return TutorialStep(
        explanation:
  ...
  getIntroCompareTutorialStep(Node a, int otherNodeIndex) {
    NodeConnection nc = getNodeConnections(a, unvisited: true).elementAt(otherNodeIndex);
